...
namespace NumericalMethods
{
    public partial class Form1 : Form
    {
        ...
        private void btnCalculate_Click(object sender, EventArgs e)
        {
            ...
            double x0, x1, x0i, x1i, tolerance;
            int iterations;
            ...
            //replace 'e' with its numeric value
            functionInput = functionInput.Replace("e", Math.E.ToString());
            ...
            do
            {
                ...
                double error = Math.Abs((xn - xni) / xn) * 100;

                //this will add the outputs in dgv
                guna2DataGridView1.Rows.Add(iteration + 1, x0i.ToString("F4"), x1i.ToString("F4"), f0.ToString("F4"), f1.ToString("F4"), xn.ToString("F4"), error.ToString("F4"));
                iteration++;

                if (error < tolerance)
                {
                    //this shows the final root in messagebox first
                    MessageBox.Show($"Final root: x = {xn:F4}");
                    //then set the visibility of the label to true, then it will replace the text of the final root
                    lblFinal.Visible = true;
                    lblFinal.Text = xn.ToString("F4");
                    return;
                }
            ...
            MessageBox.Show($"Maximum iterations reached. Last approximation: x = {xn:F4}");
            lblFinal.Visible = true;
            lblFinal.Text = xn.ToString("F4");
        }
    }
